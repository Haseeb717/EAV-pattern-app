...
module Customizable
  ...
  def set_custom_attribute(key, value)
    ...
    if attribute.save
      ...
      raise "Failed to save custom attribute: #{attribute.errors.full_messages.join(', ')}"
    end
  end
  ...
  def get_custom_attribute(key)
    @custom_attributes_cache[key] || custom_attributes.find_by(key: key)&.value
  end
  ...
  # Load and cache custom attribute definitions for validation
